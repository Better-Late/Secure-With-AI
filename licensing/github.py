...
async def _fetch_github_license_api(owner: str, repo: str) -> tuple[Optional[str], Optional[str], Optional[LicenseType]]:
    """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    if download_url:
                        async with session.get(download_url, timeout=aiohttp.ClientTimeout(total=10)) as content_response:
                            if content_response.status == 200:
                                license_text = await content_response.text()
                                license_type = _map_github_license_to_type(license_key) if license_key else LicenseType.UNKNOWN
                                return license_text, data.get('name', 'LICENSE'), license_type
...
async def get_license_opensource(github_url: str) -> Optional[License]:
    """
    ...
    try:
        ...
        license_text, found_filename, license_type = await _fetch_github_license_api(owner, repo)
