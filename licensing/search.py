...
async def _resolve_redirects(urls: List[str]) -> List[str]:
    """
    Resolve URL redirects to get final destinations concurrently.
    ...
    async def resolve_single(session: aiohttp.ClientSession, url: str) -> str:
        try:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        resolved = await asyncio.gather(*[resolve_single(session, url) for url in urls])

    return list(resolved)


async def perform_google_search(query: str, num_results: int = 3) -> List[str]:
    """
...
Only include actual webpage URLs, skip any PDF files or non-webpage results."""
        ...
        response_text = await generate_content_with_search(search_prompt)
        ...
        try:
            ...
            if isinstance(urls, list):
                ...
                # Resolve redirects
                valid_urls = await _resolve_redirects(valid_urls[:num_results])
...
async def search_licenses_via_google(brand_name: str, base_url: str) -> Optional[License]:
    """
    ...
    try:
        ...
        search_results = await perform_google_search(query, num_results=10)  # Get more to filter
